...
class TriangleMeshGeneratorImplTest {
    ...
    public void shouldCreateMeshWithAllPointsWithinGivenArea() {
        ...
        var distance = 10.0;
        var sideLength = 1000;
        var objectUnderTest = new TriangleMeshGeneratorImpl();
    ...
    @Test
    public void shouldEveryPointInMeshFormATriangle() {
        // given
        var distance = 10.0;
        var sideLength = 100;
        var objectUnderTest = new TriangleMeshGeneratorImpl();
        ...
        assertThat(mesh)
                .filteredOn(p -> p.x + distance <= sideLength && p.y + height <= sideLength)
                .allMatch(p -> set.containsAll(List.of(
                        new Point(p.x + distance, p.y),
                        new Point(p.x + half, p.y + height))));
    }
}